//Copied from Page 30028 "APIV2 - Sales Orders" from version 27.0 and added a custom field
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                field(myCustomField; Rec."My Custom Field PTE")
                {
                    Caption = 'My Custom Field';

                    trigger OnValidate()
                    begin
                        RegisterFieldSet(Rec.FieldNo("My Custom Field PTE"));
                    end;
                }
